...
public class HttpServer {
    ...
    public void run(String[] args) throws IOException, ClassNotFoundException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        ...
        String ruta = "";
        Method methodWithComp = null;
        
        Class<?> c = Class.forName(args[0]);
        for (Method m : c.getMethods()){
            if(m.isAnnotationPresent(Component.class)){
                ruta = m.getAnnotation(GetMapping.class).value();
                methodWithComp = m;
            }
        }
        
        String pathGet = "/component/hello";
        if(pathGet.startsWith("/component")){
            if(pathGet.substring(10).equals(ruta)){
                System.out.println("Salida: " + methodWithComp.invoke(null));
            }
        }
